...
$("#formEvents").submit(function(e){
    ...
    dateDebut= $.trim($("#ddEvent").val()) + " " + $.trim($("#ddtEvent").val()) ;
    ...
    action="<div class='text-center'><div class='btn-group'><button class='btn btn-primary btnEdit'>Modifier</button><button class='btn btn-danger btnDelete'>Supprimer</button></div></div>"
        ...
        if(dateFin <= dateDebut){
            ...
            if (/\s/.test(photoEvent2)) {
                alert('Le nom de photo ne peut pas contenir d\'espace. S’il vous plaît renommer la photo.');
            }
           else{
                 $.ajax('bd/crud.php', {
                  ...
                  //  processData: false, contentType: false,
                   ...
                   processData: false,
                    ...
                    console.log(data);
                    if(option == 1){
                        tableEvents.row.add([Nom_Departement,sujetEvent,descriptionEvent,dateDebut, dateFin,action]).draw();
                        Swal.fire({
                            icon: 'success',
                    ...
                    else{tableEvents.row(fila).data([ID_Evenement,Nom_Departement,sujetEvent,descriptionEvent,dateDebut, dateFin]).draw();}          
                },
...
$(document).on("click", ".btnDelete", function(){    
    ...
    var respuesta = Swal.fire({
        title: 'Vous etes sure de supprimer cet évènement?',
